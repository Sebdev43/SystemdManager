...
class ServiceDialog(ctk.CTkToplevel):
    ...
    def create_form(self):
        ...
        ctk.CTkLabel(form_frame, text="Documentation :").grid(
        ...
        service_label = ctk.CTkLabel(
            form_frame, text="[Service]", font=("Arial", 16, "bold")
        )
        ...
        type_combobox = ctk.CTkComboBox(
            form_frame, values=["simple", "forking", "oneshot", "notify"]
        )
        ...
        ctk.CTkLabel(form_frame, text="Dossier de travail :").grid(
        ...
        browse_button = ctk.CTkButton(
            ...
            text="...",
            width=40,
            command=lambda: self.browse_directory(working_dir_entry),
        )
        ...
        restart_combobox = ctk.CTkComboBox(
            form_frame,
            values=[
                "no",
                "on-success",
        ...
        ctk.CTkLabel(form_frame, text="Délai de redémarrage (s) :").grid(
        ...
        resources_label = ctk.CTkLabel(
            form_frame, text="Limites de ressources", font=("Arial", 14, "bold")
        )
        resources_label.grid(row=12, column=0, columnspan=2, pady=(20, 10), sticky="w")

        ctk.CTkLabel(form_frame, text="Limite mémoire :").grid(
        ...
        ctk.CTkLabel(form_frame, text="Quota CPU (%) :").grid(
        ...
        install_label = ctk.CTkLabel(
            form_frame, text="[Install]", font=("Arial", 16, "bold")
        )
    ...
    def create_buttons(self):
        ...
        ctk.CTkButton(button_frame, text="Annuler", command=self.cancel).grid(
            row=0, column=0, padx=10, pady=10
        )

        ctk.CTkButton(button_frame, text="Enregistrer", command=self.save).grid(
    ...
    def save(self):
        ...
        self.service.service.memory_limit = (
            self.memory_entry.get() + self.memory_unit.get()
        )
    ...
    def validate_fields(self) -> bool:
        ...
        try:
            ...
            if name and not re.match(r"^[a-zA-Z][a-zA-Z0-9-_]*$", name):
                errors.append(
                    "Le nom du service doit commencer par une lettre et ne contenir que des lettres, chiffres, tirets et underscores"
                )
            ...
            try:
                ...
                if restart_sec < 0:
                    ...
                    warnings.append(
                        "Un délai de redémarrage supérieur à 5 minutes pourrait être problématique"
                    )
            except ValueError:
            ...
            try:
                ...
                if not 0 <= cpu_quota <= 100:
                    ...
                    warnings.append(
                        "Un quota CPU très bas pourrait affecter les performances du service"
                    )
            except ValueError:
            ...
            if memory_value:
                try:
                    ...
                    if memory <= 0:
                        ...
                        warnings.append(
                            "Une limite de mémoire très élevée pourrait affecter le système"
                        )
                except ValueError:
            ...
            if working_dir:
                if not os.path.isabs(working_dir):
                    ...
                    warnings.append(
                        "Le dossier de travail pourrait ne pas être accessible"
                    )
            ...
            if not exec_start:
                ...
                if cmd_parts:
                    ...
                    if not os.path.isabs(executable):
                        errors.append(
                            "La commande ExecStart doit utiliser un chemin absolu"
                        )
                        ...
                        if not os.path.isfile(executable):
                            ...
                            if not executable.endswith((".sh", ".py", ".bash", ".js")):
                                warnings.append(
                                    f"'{executable}' n'a pas les permissions d'exécution"
                                )
            ...
            if warnings:
                ...
                result = messagebox.askokcancel(
                    "Avertissements", warning_message + "\n\nVoulez-vous continuer ?"
                )
                return result
