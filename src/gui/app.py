...
from src.i18n.translations import _, i18n
...
class SystemdManagerApp(ctk.CTk):
    ...
    def __init__(self):
        ...
        if not self.cli_controller.check_sudo():
            self.cli_controller.request_sudo(
                "Ce programme nécessite les droits sudo pour gérer les services systemd."
            )
    ...
    def create_sidebar(self):
        ...
        refresh_btn = ctk.CTkButton(
            sidebar, text="🔄 Actualiser", command=self.refresh_current_tab
        )
    ...
    def create_main_view(self):
        ...
        self.lang_button = ctk.CTkButton(
            title_frame, text="FR/EN", width=60, height=30, command=self.change_language
        )
            ...
            if isinstance(widget, ctk.CTkFrame):
                for child in widget.winfo_children():
                    if isinstance(child, ctk.CTkSwitch):
                        child.configure(
                            text="Mode sombre" if new_mode == "dark" else "Mode clair"
                        )
                        if new_mode == "dark":
                            child.select()
                        else:
    ...
    def update_translations(self):
        ...
        for widget in self.winfo_children():
            if isinstance(widget, ctk.CTkFrame):
                for child in widget.winfo_children():
                    if (
                        isinstance(child, ctk.CTkButton)
                        and child not in self.nav_buttons
                    ):
