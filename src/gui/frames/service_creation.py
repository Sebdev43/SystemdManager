...
class ServiceCreationFrame(ctk.CTkFrame):
    ...
    def __init__(self, parent):
        ...
        self.user_var = ctk.StringVar(
            value=os.getenv("SUDO_USER", os.getenv("USER", "root"))
        )
    ...
    def create_form(self):
        ...
        ctk.CTkLabel(base_info_frame, text=_("Description")).grid(
            row=row, column=0, **self.padding, sticky="w"
        )
        ctk.CTkEntry(base_info_frame, textvariable=self.description_var).grid(
            row=row, column=1, **self.padding, sticky="ew"
        )
        ctk.CTkLabel(
            base_info_frame,
            text=_(
        ...
        ctk.CTkLabel(base_info_frame, text=_("Service Type")).grid(
            row=row, column=0, **self.padding, sticky="w"
        )
        ...
        exec_config_frame.grid(
            row=row, column=0, columnspan=2, sticky="ew", padx=10, pady=5
        )
        exec_config_frame.grid_columnconfigure(1, weight=1)

        ctk.CTkLabel(exec_config_frame, text=_("User")).grid(
            row=row, column=0, **self.padding, sticky="w"
        )
        users = self.get_system_users()
        user_menu = ctk.CTkOptionMenu(
            exec_config_frame, values=users, variable=self.user_var
        )
        ...
        ctk.CTkLabel(dir_frame, text=_("Working Directory")).grid(
        ...
        ctk.CTkButton(
            dir_frame, text="📂", width=50, command=self.browse_directory
        ...
        self.command_frame = ctk.CTkFrame(exec_config_frame, fg_color="transparent")
        ...
        ctk.CTkButton(
            executable_container, text="🔄", width=30, command=self.refresh_executables
        ).grid(row=0, column=1)
        ...
        advanced_frame = ctk.CTkFrame(self.scrollable_frame, fg_color="transparent")
        advanced_frame.grid(
            row=row, column=0, columnspan=2, sticky="nsew", padx=10, pady=(15, 5)
        )
        ...
        self.restart_sec_label = ctk.CTkLabel(
            advanced_frame, text=_("Restart Delay (sec)")
        )
        self.restart_sec_label.grid(row=row, column=0, **self.padding, sticky="w")
        ctk.CTkEntry(advanced_frame, textvariable=self.restart_sec_var, width=300).grid(
            row=row, column=1, **self.padding, sticky="ew"
        )
        ...
        self.restart_sec_help.grid(
            row=row + 1, column=1, **self.help_padding, sticky="w"
        )
        row += 2

        self.start_delay_label = ctk.CTkLabel(
            advanced_frame, text=_("Start Delay after boot (sec)")
        )
        self.start_delay_label.grid(row=row, column=0, **self.padding, sticky="w")
        ctk.CTkEntry(advanced_frame, textvariable=self.start_delay_var, width=300).grid(
            row=row, column=1, **self.padding, sticky="ew"
        )
        self.start_delay_help = ctk.CTkLabel(
            advanced_frame,
            text=_(
        ...
        self.start_delay_help.grid(
            row=row + 1, column=1, **self.help_padding, sticky="w"
        )
        row += 2

        self.max_restarts_label = ctk.CTkLabel(
            advanced_frame, text=_("Maximum number of restarts")
        )
        self.max_restarts_label.grid(row=row, column=0, **self.padding, sticky="w")
        ctk.CTkEntry(
            advanced_frame, textvariable=self.max_restarts_var, width=300
        ).grid(row=row, column=1, **self.padding, sticky="ew")
        self.max_restarts_help = ctk.CTkLabel(
            advanced_frame,
            text=_("Maximum number of restarts allowed in 5 minutes\nDefault: 3"),
            text_color="gray60",
            font=("", 10),
        )
        self.max_restarts_help.grid(
            row=row + 1, column=1, **self.help_padding, sticky="w"
        )
        row += 2
        ...
        self.start_after_save_label.grid(
            row=row, column=0, columnspan=2, **self.padding, sticky="w"
        )
        row += 1
    ...
    def create_section_label(self, text, row):
        ...
        section_frame.grid(
            row=row, column=0, columnspan=2, sticky="ew", padx=10, pady=(10, 5)
        )
        self.section_label = ctk.CTkLabel(
            section_frame, text=text, font=("", 16, "bold")
        )
    ...
    def create_buttons(self):
        ...
        button_frame.grid(
            row=1000, column=0, columnspan=2, sticky="ew", padx=10, pady=20
        )
        button_frame.grid_columnconfigure(0, weight=1)
        button_frame.grid_columnconfigure(1, weight=1)

        self.cancel_button = ctk.CTkButton(
            button_frame, text=_("Annuler"), command=self.cancel_creation
        )
        self.cancel_button.grid(row=0, column=0, padx=5, pady=5, sticky="e")

        self.create_button = ctk.CTkButton(
            button_frame, text=_("Créer"), command=self.create_service
        )
    ...
    def create_service(self):
        try:
            ...
            try:
                start_limit_burst = (
                    int(config["max_restarts"]) if config["max_restarts"] else 3
                )
            ...
            if self.start_after_save_var.get():
                try:
                    subprocess.run(
                        ["systemctl", "start", f"{service.name}.service"], check=True
                    )
                except subprocess.CalledProcessError as e:
                    self.show_error(
                        _("Erreur lors du démarrage du service : ") + str(e)
                    )
                    return
    ...
    def validate_working_directory(self, event=None):
        ...
        try:
            if not os.path.exists(working_dir):
                self.working_dir_var.set("")
                self.after(
                    100, lambda: self.show_error(_("Le dossier spécifié n'existe pas."))
                )
                return False
            ...
            if not os.path.isdir(working_dir):
                ...
                self.after(
                    ...
                    lambda: self.show_error(
                        _("Le chemin spécifié n'est pas un dossier.")
                    ),
                )
                return False
            ...
            if not os.access(working_dir, os.R_OK):
                ...
                self.after(
                    100,
                    lambda: self.show_error(
                        _("Vous n'avez pas les permissions pour accéder à ce dossier.")
                    ),
                )
                return False
            ...
            self.working_dir_var.set("")
            # Capture the message now: `e` is cleared at the end of the except
            # block, but this lambda runs later via self.after().
            message = _("Erreur lors de la validation du dossier : ") + str(e)
            self.after(100, lambda: self.show_error(message))
            return False
    ...
    def show_error(self, message):
        ...
        try:
            ...
            ctk.CTkLabel(dialog, text=_("⚠️ ") + message, wraplength=350).pack(
                padx=20, pady=20
            )

            ctk.CTkButton(dialog, text=_("OK"), command=dialog.destroy).pack(pady=10)
    ...
    def validate_manual_command(self, event=None):
        ...
        if not os.access(executable, os.X_OK) and not executable.endswith(
            (".sh", ".py", ".bash", ".js")
        ):
    ...
    def on_executable_selected(self, value):
        ...
        if value not in [
            "Sélectionnez d'abord un dossier de travail",
    ...
    def refresh_executables(self):
        ...
        working_dir = self.working_dir_var.get()
        if not working_dir:
            self.executable_menu.configure(
    ...
    def update_command(self):
        ...
        if self.command_method_var.get() == "browse":
            ...
            if executable and executable not in [
                "Sélectionnez d'abord un dossier de travail",
                "Aucun exécutable trouvé",
                "Erreur de lecture",
            ]:
        ...
        try:
            ...
            if hasattr(self, "description_label"):
                ...
                self.description_help.configure(
                    text=_("Une brève description du service")
                )

            if hasattr(self, "type_label"):
                self.type_label.configure(text=_("Type de service") + " *")
                self.type_help.configure(
                    text=_("Le type détermine le comportement du service")
                )

            if hasattr(self, "user_label"):
                self.user_label.configure(text=_("Utilisateur") + " *")
                self.user_help.configure(
                    text=_("L'utilisateur qui exécutera le service")
                )

            if hasattr(self, "working_directory_label"):
                self.working_directory_label.configure(
                    text=_("Répertoire de travail") + " *"
                )
                self.working_directory_help.configure(
                    text=_("Le répertoire de travail du service")
                )
            ...
            if hasattr(self, "restart_label"):
                ...
                self.restart_help.configure(
                    text=_("Redémarre automatiquement en cas d'échec")
                )

            if hasattr(self, "restart_sec_label"):
                self.restart_sec_label.configure(
                    text=_("Délai avant redémarrage (secondes)")
                )
                self.restart_sec_help.configure(
                    text=_("Temps d'attente avant redémarrage")
                )

            if hasattr(self, "max_restarts_label"):
                self.max_restarts_label.configure(
                    text=_("Nombre maximum de redémarrages")
                )
                self.max_restarts_help.configure(
                    text=_("Nombre maximum de tentatives de redémarrage")
                )
